...
myApp.onPageInit('signup-email', function(page) {
	...
	/* Basic Calendar */
	var calendarBasic = myApp.calendar({
    input: '.page[data-page=signup-email] #calendar-basic'
	});

	/* Calendar with Custom Date Format */
	var calendarCustomDateFormat = myApp.calendar({
    dateFormat: 'DD, MM dd, yyyy',
		input: '.page[data-page=signup-email] #calendar-custom-date-format'
	});

	/* Calendar with Multiple Dates */
	var calendarMultipleDates = myApp.calendar({
    dateFormat: 'M dd, yyyy',
		input: '.page[data-page=signup-email] #calendar-multiple-dates',
    multiple: true
	});
	...
	var calendarDateRange = myApp.calendar({
    dateFormat: 'dd MM yyyy',
		input: '.page[data-page=signup-email] #calendar-date-range',
    rangePicker: true
	});
    ...
    disabled: {
      ...
      to: dateWeekLater
		...
		input: '.page[data-page=signup-email] #calendar-disabled-dates'
	});
...
myApp.onPageInit('tour-guide', function(page) {

	$$('body').on('click', '.page[data-page=tour-guide] [data-action=start-tour]', function(e) {
		e.preventDefault();
		...
		var tourSteps = [
			{
				step: 1,
				element: '.page[data-page=tour-guide] .navbar .left',
				header: '<b>1<sup>st</sup> Tour Step</b>',
				message: 	"This is our first tour step." +
									"<br/>" +
									"Please click 'Next' to move forward."
			},
			{
        step: 2,
				element: '.page[data-page=tour-guide] [data-action=start-tour]',
        header: '<b>2<sup>nd</sup> Tour Step</b>',
        message: 	"This is our second tour step." +
									"<br/>" +
									"You can also use icons or images." +
									'<p class="text-center"><img src="assets/custom/img/logo.svg" width="96" alt="" /></p>' +
									"Please click 'Next' or 'Previous' to move forward or backward respectively."
			},
			{
        step: 3,
				element: '.page[data-page=tour-guide] .navbar .center',
        header: '<b>Final Tour Step</b>',
        message: 	"Congratulations! You have finished the tour guide tutorial. Enjoy"
			},
		];

		var tourGuide = myApp.tourguide(tourSteps, tourOptions);
